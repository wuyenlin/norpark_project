...
class Experiment(object):
    ...
    def get_argmax_iters(self):
        ...
        it_idx = [argmax(outs['accuracy']) for k, outs in list(log_data['test']['out'].items())]
    ...
    def get_max_min_val_iter(self):
        log_data = self.get_log_data()
        accuracies = np.array([outs['accuracy'] for k, outs in list(log_data['test']['out'].items())])
    ...
    def summarize(self, datasets=None, exact=True, show_train_points=True, mode='maxone'):
        ...
        if exact:
            ...
            pdata = [[round(outs['accuracy'][i], 4) for i in it_idx] for k, outs in list(log_data['test']['out'].items())]
